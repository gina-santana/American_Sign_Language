...
def data_generator():
    ...
    train = pd.read_csv('../data/stacked_trial/sign_mnist_train.csv')
    y = train['label'].values
    X = train.drop('label', axis=1).values.reshape(-1,28,28,1)
    train_gen = train_datagen.flow(X, y, subset='training')
    valid_gen = train_datagen.flow(X, y, subset='validation')
...
if __name__=='__main__':
    ...
    model = cnn_model()
    train_gen, valid_gen= data_generator()
    model.summary()
